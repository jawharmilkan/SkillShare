...
// Fetch accepted connections (either as requester or skill owner)
$sql = "
    SELECT 
        r.*, 
...
<head>
  ...
  <title>SkillShare - My Connections</title>
...
<body class="bg-gradient-to-r from-blue-50 to-teal-50 min-h-screen">
  ...
  <main class="max-w-4xl mx-auto py-16 px-4">
    <div class="bg-white rounded-3xl shadow-xl p-8">
      <h1 class="text-3xl font-extrabold text-blue-700 mb-6 text-center">My Connections</h1>
      <?php if ($res && $res->num_rows > 0): ?>
        <div class="space-y-6">
          <?php while ($row = $res->fetch_assoc()): 
            ...
            ?>
            <div class="flex items-center gap-5 bg-blue-50 rounded-xl p-5 shadow">
              ...
              <a href="chat.php?request_id=<?php echo $row['id']; ?>" class="bg-blue-600 text-white px-5 py-2 rounded-full shadow hover:bg-blue-700 font-bold">Chat</a>
            </div>
          <?php endwhile; ?>
        </div>
      <?php else: ?>
        <div class="text-center text-gray-500 py-10 text-xl">You don't have any connections yet.</div>
      <?php endif; ?>
    </div>
  </main>
